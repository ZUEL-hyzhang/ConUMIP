...
class DistributionUncertainty(nn.Module):
    ...
    def forward(self, x):
        def mixup(x, dsu_x):
            ...
            mixed_x = lam * x + (1 - lam) * dsu_x
            return mixed_x

        def mask_mixup(x, dsu_x, mask):

            mixed_x = (1-mask) * x + mask * dsu_x
            return mixed_x

        def generate_mask_random(lam, x,args):
            mask_token_num_start=0
            height = x.shape[0]
            width = x.shape[1]
            ...
            mask_idx = np.random.permutation(height * width)[:num_masking_patches]
            mask[mask_idx] = 1
            mask = mask.reshape(height, width)
            mask = torch.from_numpy(mask).float().to(args.device)
            lam = 1 - num_masking_patches / (height * width)
            return mask, lam
        ...
        lam = np.random.beta(1.0, 1.0)
        # mask, lam= generate_mask_random(lam, x,args)
        ...
        # x = (x - mean.reshape(1,x.shape[1])) / std.reshape(1,x.shape[1])
        # x_dsu = x * gamma.reshape(-1, x.shape[1]) + beta.reshape(-1,x.shape[1])
        x_dsu = x * gamma.reshape(x.shape[0], -1) + beta.reshape(x.shape[0], -1)
        # idx = np.random.permutation(x.shape[0])
        # x_per=x_dsu[idx]
        # x = mask_mixup(x_dsu, x_per, mask)
        return x
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = ChordMixer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                        time_feat_dim=args.time_feat_dim, num_tokens=args.num_neighbors, num_layers=args.num_layers, dropout=args.dropout, device=args.device)
            ...
            dynamic_backbone = DyGMixer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim, patch_size=args.patch_size,
                                         num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                         max_input_sequence_length=args.max_input_sequence_length, device=args.device)
        else:
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.aug=='UMIP':
                    ...
                    positive_probabilities = model[-1](aug_positive_emb, stat='pos').squeeze(dim=-1).sigmoid()
                    negative_probabilities = model[-1](aug_negative_emb, stat='neg').squeeze(dim=-1).sigmoid()
                    pred_loss = loss_func(positive_probabilities, torch.ones_like(positive_probabilities)) + loss_func(negative_probabilities, torch.zeros_like(negative_probabilities))
                    # aug_positive_emb=aug_layer(positive_emb)
                    # positive_probabilities = model[-1](aug_positive_emb, stat='pos').squeeze(dim=-1).sigmoid()
                    # negative_probabilities = model[-1](negative_emb, stat='neg').squeeze(dim=-1).sigmoid()
                    # pred_loss = loss_func(positive_probabilities, torch.ones_like(positive_probabilities)) + loss_func(negative_probabilities, torch.zeros_like(negative_probabilities))
                    # # positive_emb=model[-2](positive_emb)
                    # aug_positive_emb=model[-2](aug_positive_emb)
                    # contrastive_loss=loss_cl(positive_emb,aug_positive_emb)
                    loss=pred_loss
                    ...
                    positive_emb=model[-2](positive_emb)
                    aug_positive_emb=model[-2](aug_positive_emb)
                    negative_emb=model[-2](negative_emb)
                    aug_negative_emb=model[-2](aug_negative_emb)
                    aug_emb=torch.cat([aug_positive_emb,aug_negative_emb],dim=0)
                    origin_emb=torch.cat([positive_emb,negative_emb],dim=0)
                    contrastive_loss=loss_cl(origin_emb,aug_emb)

                    # contrastive_loss=loss_cl(positive_emb,aug_positive_emb)+loss_cl(negative_emb,aug_negative_emb)
                    loss=pred_loss+1*contrastive_loss
                else:
